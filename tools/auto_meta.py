...
Кому это нужно. Запись, загруженную через сайт (`app/api/ingest.py`), человек описывает двумя
...
разметку делает сервер, шагом `ingest.after`, СРАЗУ после сборки и ДО индексации: поля шапки
