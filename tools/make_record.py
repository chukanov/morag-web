...
def build_one(args, source: Path, changed_out: list | None = None) -> int:
    ...
    # разбора экрана, приём записи с чужой машины), а «запись будет без плеера» там — неправда:
    # видео приедет своим путём. Пугать предупреждением о сделанном выборе незачем.
    if not meta["media"] and not args.no_media:
