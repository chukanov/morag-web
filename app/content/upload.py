...
транскрибации на своём Mac (`tools/upload.py`) и загружает сюда пакет: сырой артефакт адаптера,
...
`names.json` подписал бы его чужим именем. Сдвиг в отдельный диапазон (`upload.speaker_base` +
...
async def accept(staging: Staging, rid: str, *, family: Path, cfg, root: Path,
    ...
    Индексация — отдельной задачей той же очереди (см. `api/upload.py`): она длинная.
    """
    d = staging.dir(rid)
