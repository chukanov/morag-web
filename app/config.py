...
class IngestLlmCfg(BaseModel):
    ...
    `slots` — сколько запросов ОДНОГО человека идут к шлюзу разом (адаптер шлёт до восьми, а BFF
    это один процесс: без потолка расшифровка заткнула бы живым посетителям `/api/ask`).
    Лишние ЖДУТ очереди `queue_timeout` секунд, а не получают отказ: упавшая стадия стоит
    дороже, чем минута ожидания.
    """

    enabled: bool = True
    slots: int = 4
    queue_timeout: float = 180     # сколько стадия готова простоять в очереди
    timeout: float = 300           # описание кадра Vision занимает десятки секунд
...
class AccessCfg(BaseModel):
    ...
    как в shell): доменный пользователь входит, только если состоит хотя бы в одной. `users` —
    логины ПОИМЁННО, поверх масок (17.09): коллега из другого подразделения, которому сайт нужен,
    а группы у него нет; сравнивается с логином каталога без учёта регистра и домена. Пусто и
    там, и там — любой вошедший. Локальные пользователи (`auth.users`) проходят всегда: они и
    заводятся для того, чтобы пустить того, кого нет в каталоге. Проверяется и при входе (403 с
    `message`), и на каждом запросе по снимку учётки — ужесточили правило (сняли группу, вычеркнули
    логин), чужие сессии кончились сразу.
    """
