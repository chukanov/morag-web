...
export async function renderIngest() {
  ...
  const zip = el("a", { class: "ing-get ghost", href: data.app, text: "Скачать файлом" });
  mount.replaceChildren(
    el("p", { class: "ing-lead" },
    ...
    el("p", { class: "ing-note", text: `При установке приедет ${gb(data.bytes || 0)}. Ссылка живёт ${data.days} дней — потом откройте эту страницу заново.` }),

    el("h3", { class: "ing-h", text: "Если строка в терминале пугает" }),
    el("div", { class: "ing-get-row" }, zip,
      el("span", { class: "ing-note", text: "тот же установщик файлом" })),
    el("p", { class: "ing-note" },
      "Распакуйте — и ", el("b", { text: "перетащите файл в окно Терминала, затем Enter" }),
      ". Двойной щелчок macOS не пропустит: приложение не подписано в Apple, и на всё скачанное "
      + "она отвечает отказом (обойти можно и через «Системные настройки → Конфиденциальность и "
      + "безопасность → Всё равно открыть», но это дольше)."),
